...
constructs. As a real-world example, take ``subprocess.Popen``'s ``__init__`` method from the official ``_typeshed``
repository (`source <https://github.com/python/typeshed/blob/eb347f3a1c13dc00449bf178a53c24f2ae838450/stdlib/subprocess.pyi#L1472>`__):
...
As you can see, to properly type the various combinations of ``universal_newlines``, ``text``, ``encoding``,
...
we would need a total of 36 overloads [#popen-overloads]_!

With the proposed mechanism, this could be expressed with two ``OverloadedType``'s and a single declaration for ``__init__``.
...
semantics defined in :ref:`typing: Overload <typing:overload>`.
